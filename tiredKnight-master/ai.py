...
def maximizer(board: chess.Board, depth: int, alfa,beta, evaluate=vanilla_eval) -> float:
    ...
    for move in board.legal_moves:
        board.push(move)
        value = max(value, minimizer(board, depth - 1, alfa,beta, evaluate))
        board.pop()
        if value >= beta:
            return value
        alfa = max(alfa, value)
...
def minimizer(board: chess.Board, depth: int, alfa,beta, evaluate=vanilla_eval) -> float:
    if board.is_checkmate():
        return float("inf")
    if depth == 0 or board.is_game_over():
        return evaluate(board)
    ...
    for move in board.legal_moves:
        board.push(move)
        value = min(value, maximizer(board, depth - 1, alfa,beta, evaluate))
        board.pop()
        if value <= alfa:
            return value
        beta = min(beta, value)
